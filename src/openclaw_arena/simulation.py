...
def _equity(portfolio: Portfolio, price: float, token_value: float) -> float:
    return portfolio.cash + portfolio.position * price + token_value
...
def run_simulation(steps: int = 100, seed: int = 1, snapshot_path: str | None = None) -> dict:
    ...
    for state in snapshots:
        ...
        for ctx in agents:
            ...
            if proposal is not None:
                ...
                if approved:
                    ok, launch_status = venue.launch_token(
                        ticker=proposal.ticker,
                        creator=proposal.agent_id,
                        initial_supply=proposal.initial_supply,
                        seed_price=max(0.0001, state.price * (1 + state.momentum)),
                    )
                    status = launch_status if ok else launch_status
                    if ok:
        ...
        # Venue-level token trading across all launched tokens by all agents.
        for ticker, market in venue.markets.items():
            for ctx in agents:
                inventory = ctx.portfolio.token_inventory.get(ticker, 0.0)
                order = ctx.agent.propose_token_order(state, ticker, market.price, inventory)
                if order is None:
                    continue
                fill = venue.process_order(order, ctx.portfolio)
                token_trade_logs.append(
                    {
                        "step": step,
                        "agent": ctx.agent.agent_id,
                        "ticker": ticker,
                        "side": order.side.value,
                        "requested_qty": round(order.quantity, 4),
                        "filled_base_qty": round(fill.filled_base_qty, 6),
                        "quote_delta": round(fill.quote_delta, 6),
                        "execution_price": round(fill.execution_price, 8),
                        "status": fill.reason,
                        "inventory_after": round(ctx.portfolio.token_inventory.get(ticker, 0.0), 6),
                    }
                )
        ...
        for ctx in agents:
            ...
            token_value = venue.mark_to_market_value(ctx.portfolio)
            equity = _equity(ctx.portfolio, state.price, token_value)
            ...
            logs.append(
                {
                    ...
                    "token_value": round(token_value, 4),
    ...
    for ctx in agents:
        token_value = venue.mark_to_market_value(ctx.portfolio)
        final_equity = _equity(ctx.portfolio, price, token_value)
        ...
        scoreboard.append(
            {
                ...
                "token_inventory": {k: round(v, 6) for k, v in sorted(ctx.portfolio.token_inventory.items()) if abs(v) > 1e-12},
            }
        )
    ...
    token_markets = {
        ticker: {
            ...
            "base_reserve": round(market.base_reserve, 6),
            "quote_reserve": round(market.quote_reserve, 6),
        }
        for ticker, market in sorted(venue.markets.items())
    }
    return {
        "steps": len(snapshots),
        "seed": seed,
        ...
        "token_trade_logs": token_trade_logs,
    }
